...
- Name: components.spec.ts >> Metronome Component Tests >> metronome has start/stop button
- Location: tests/components.spec.ts:51:3

# Error details

```
Error: locator.isVisible: Error: strict mode violation: getByText('Play') resolved to 3 elements:
    1) <h2 class="text-4xl font-black tracking-tight">Free Play</h2> aka getByRole('button', { name: 'Free Play Just jam! No rules' })
    2) <p class="text-sm font-medium text-white/80">Play fun mini-games</p> aka getByRole('button', { name: 'Arcade Play fun mini-games' })
    3) <p class="text-sm font-medium text-white/80">Real-time multiplayer</p> aka getByRole('button', { name: 'Duet Mode Real-time' })

Call log:
    - checking visibility of getByText('Play')
